/*
 * Original code from https://github.dev/ivandotv/estacion (MIT License)
 */

import type { EventPayload } from './broadcaster';
...
export class EventBus {
  ...
  constructor(config?: EventBusConfig) {
    this._maxListeners = config?.maxListeners ?? 0;
  ...
  /* when any channel emits, re-emit on default channel  */
  private onChannelEmit(payload: EventPayload): void {
    ...
    if (this.defaultChannel.hasTopic(topicName)) {
      ...
      this.defaultChannel.topic(topicName).getEventEmitter().emit(topicName, topicPayload);
    }
    this.defaultChannel.getEventEmitter().emit(this.defaultChannel.name, payload); // for all subscribers
  }
